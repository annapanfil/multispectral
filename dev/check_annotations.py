...
def draw_boxes_one_class(
    pool_image, image_path, label_path, filter_class, color, image_shape, correction, scale
):
        ...
        for photo in wrong:
            ...
            if len(wrong[photo]) == 0:
                ...
                for name, n_obj, correct_n_obj in wrong[photo]:
                    f.write(
                        f"{n_obj} {name} instead of {correct_n_obj} ({'too little' if n_obj < correct_n_obj else 'too many'})\n"
                    )
    print(f"Summary saved to {out_path}/{group}_summary.txt")


def show_class_in_normalised_pool(
    photo_names, n_objects, raw_palette, image_bbs, group, config, class_id, out_path="out"
):
    plt.figure(figsize=(20, 8))
    legend_elements = [
        Patch(facecolor=color, edgecolor="black", label=f"photo {name}, objects: {n_obj}")
        for color, name, n_obj in zip(raw_palette[: i + 1], photo_names, n_objects)
    ]
...
if __name__ == "__main__":
    ...
    image_fns = [
        image_fn
        for image_fn in os.listdir(f"{path}/images/train")
        if image_fn.endswith("_meanRE.png")
    ]
    image_fns.sort()
    ...
    image_shape = cv2.imread(f"{path}/images/train/{image_fns[0]}", cv2.COLOR_BGR2GRAY).shape[
        :2
    ...
    for group in groups:
        ...
        if len(group_fns) > len(palette):
            print("Not enough distinct colours. Reusing colours.")

        for class_id in range(len(config["names"])):
            image_bbs = np.zeros(
                (pool_height, pool_width, 3), dtype=np.uint8
            )  # representing the pool
            n_objects = []

            for i, image_fn in enumerate(group_fns):
                correction, scale = get_correction_and_scale(
                    path, image_fn, pool_width, pool_height
                )

                # show bbs
                image_bbs, n_obj = draw_boxes_one_class(
                    image_bbs,
            ...
            wrong_n_obj = [
                (photo, n_obj)
                for photo, n_obj in zip(photo_names, n_objects)
                if n_obj != most_common_n
            ]
            ...
            print(
                f"{config['names'][class_id]}: {most_common_n} objects ({min(n_objects)}-{max(n_objects)})"
            )

            # if len(wrong_n_obj) > 0:
            show_class_in_normalised_pool(
                photo_names, n_objects, raw_palette, image_bbs, group, config, class_id, out_path
            )
        ...
        with open(f"{out_path}/{group}_objects.txt", "w") as f:
            for class_id, n_obj in objects.items():
                f.write(
                    f"{config['names'][class_id]}: {n_obj[0]} objects ({n_obj[1]} - {n_obj[2]})\n"
                )
